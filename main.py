...
class PairForm(FlaskForm):
    global names_dict
    name1 = SelectField('Person 1', choices=[(i, name) for i, name in names_dict.items()], validators=[DataRequired()])
    name2 = SelectField('Person 2', choices=[(i, name) for i, name in names_dict.items()], validators=[DataRequired()])
    like = SelectField('Like', choices=[('yes', 'Yes'), ('no', 'No')], validators=[DataRequired()])
...
def preferences():
    ...
    if form.validate_on_submit():
        # Process the data
        pairs_data = [(pair['name1'], pair['name2'], pair['like']) for pair in form.pairs.data]
