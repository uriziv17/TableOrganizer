...
def updatePrMatrix(table, prMatrix,personsMatrix):
    ...
    for spot in range (numRows):
        ...
        for person in range (numColumns):
            res[spot][person] = prMatrix[spot][person]*calculateSupport(table, personsMatrix, person, spot, prMatrix )
...
def calculateR(table: Table, personsMatrix, person1, spot1, person2, spot2):
    ...
    if spot1==spot2:
        ...
        return abs(0.9-abs(personsMatrix[person1][person2]-table.get_relation_between(table, spot1, spot2)))
...
def calculateSupport(table, personsMatrix, person1, spot1, prMatrix):
    ...
    for spot in table.spots:
        for person in personsMatrix[person1]:
            support = support + calculateR(table, personsMatrix, person1, spot1, person, spot) * prMatrix[spot][person]
    return support
